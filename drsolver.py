from z3 import *
solver=Solver()
i = Int("i")
k = Int("k")
j = Int("j")
v = Int("v")
a = Int("a")
i_drdVar_10 = Int("i_drdVar_10")
wr_arr_index_0 = Int("wr_arr_index_0")
wr_cond_0 = And (wr_arr_index_0 == (i_drdVar_10), ( i_drdVar_10  >= 0) , ( i_drdVar_10 < 1000), True)
waws = False

i_drdVar_14 = Int("i_drdVar_14")
r_arr_index_0 = Int("r_arr_index_0")
r_cond_0 = And (r_arr_index_0 == (i_drdVar_14 + 2), ( i_drdVar_14  >= 0) , ( i_drdVar_14 < 1000), True)
raw_cond_0 = And( wr_cond_0, (wr_arr_index_0 == r_arr_index_0), r_cond_0, (i_drdVar_10 != i_drdVar_14))
raws = Or(raw_cond_0)
cstrnts = Or(waws,raws)
solver.add(cstrnts)
if solver.check() == z3.sat:
	print("data race(waw/raw/war) exists within the loop!")
else:
	print("seems like no data race exists (please double check)")
